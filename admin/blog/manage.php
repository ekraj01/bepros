...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_post'])) {
    ...
    if ($title && $content) {
        ...
        if (!$error) {
            try {
                $stmt = $pdo->prepare("INSERT INTO blog_posts (title, content, image, is_published, created_at) VALUES (?, ?, ?, ?, NOW())");
                $stmt->execute([$title, $content, $image, $is_published]);
...
<head>
    ...
    <title>Manage Blogs - BePros Admin</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
    <?php if (!file_exists('assets/css/style.css')): ?>
        <style>/* Fallback styles if CSS file is missing */</style>
    <?php endif; ?>
...
<body>
        ...
        <div class="container">
            ...
            <div class="collapse navbar-collapse" id="navbarNav">
                <ul class="navbar-nav ms-auto">
                    ...
                    <li class="nav-item"><a class="nav-link active" href="manage.php">Blogs</a></li>
    ...
    <div class="container mt-5">
        ...
        <form method="POST" enctype="multipart/form-data">
            ...
            <button type="submit" name="add_post" class="btn btn-primary">Add Blog Post</button>
        </form>
                ...
                <?php else: ?>
                    <?php foreach ($blog_posts as $post): ?>
                        <tr>
                            ...
                            <td><?php echo htmlspecialchars($post['image'] ?: 'No Image'); ?></td>
                            <td><?php echo $post['is_published'] ? 'Yes' : 'No'; ?></td>
                            <td>
                                <a href="edit.php?id=<?php echo $post['id']; ?>" class="btn btn-warning btn-sm">Edit</a>
                                <a href="manage.php?delete=<?php echo $post['id']; ?>" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure you want to delete this blog post?')">Delete</a>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                <?php endif; ?>
